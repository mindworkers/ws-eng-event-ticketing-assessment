...
export const transfersAPI = {
  create: (token: string, data: { bookingId: string; recipientEmail: string }) =>
    fetchAPI<{ success: boolean; data: import("@/types").Booking; message?: string }>("/api/transfers", {
      method: "POST",
      body: JSON.stringify(data),
      token,
    }),
...
export const waitlistAPI = {
  join: (token: string, data: { eventId: string }) =>
    fetchAPI<{ success: boolean; data: { bookingId: string; position: number }; message?: string }>(
      "/api/waitlist/join",
      {
        method: "POST",
        body: JSON.stringify(data),
  ...
  leave: (token: string, data: { eventId: string }) =>
    fetchAPI<{ success: boolean; message?: string }>("/api/waitlist/leave", {
      method: "POST",
      body: JSON.stringify(data),
      token,
    }),
  ...
  position: (token: string, eventId: string) =>
    fetchAPI<{ success: boolean; data: { bookingId: string; position: number } }>(
      `/api/waitlist/position?eventId=${eventId}`,
      { token },
    ),
};
